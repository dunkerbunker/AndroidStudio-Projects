    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 101) {
            if (resultCode == RESULT_OK) {
                ...
                // show toast to confirm note was added
                StyleableToast.makeText(this, "Note Added!", R.style.pinkToast).show()
            }
            ...
            if (resultCode == RESULT_OK) {
                ...
                // show toast to confirm note was updated
                StyleableToast.makeText(this, "Note Updated!", R.style.pinkToast).show()
            }
        }
    }
        ...
        when (item?.itemId) {
            ...
            R.id.pin -> {
                if (selectedNote.isPinned()) {
                    ...
                    StyleableToast.makeText(this, "Note Unsaved!", R.style.redToast).show()
                } else {
                    // if the note is not pinned then pin it
                    database.mainDAO().pin(selectedNote.getID(), true)
                    // tell user that the note is pinned
                    StyleableToast.makeText(this, "Note has been saved!", R.style.pinkToast).show()
                }
                // clear list and get all notes from database
                notes.clear()
                notes.addAll(database.mainDAO().getAll())
            ...
            R.id.delete -> {
                ...
                StyleableToast.makeText(this, "Note Deleted!", R.style.redToast).show()
            ...
            R.id.share -> {
                // get values from the note
                val title = selectedNote.getTitle()
                val notes = selectedNote.getNotes()

                // create a new intent to share the note
                ...
                if (intent.resolveActivity(this.packageManager) != null) {
                    ...
                    // if there is no app that can handle the intent
                    StyleableToast.makeText(this, "No email app to share with!", R.style.redToast).show()
                }
